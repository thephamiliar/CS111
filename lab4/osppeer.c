...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	strncpy(t->filename, filename, FILENAMESIZ); // JOSH: TASK 2:  protect against buffer overrun
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	for (i = 0; i < 50; i++) {
		if (i == 0)
			strncpy(t->disk_filename, t->filename, FILENAMESIZ); // JOSH: TASK 2: protect against buffer overflow
		else
